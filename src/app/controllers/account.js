...
const { updateValidator, createSocialLink } = require("../utils/validator/account");
/*
...
module.exports.createSocialLink  = async(req, res) =>{
  ...
  if (error)
    ...
    if (result.rowCount == 0)
      return res.status(400).json({ message: "Can not create this link for unknown reasons" });
...
module.exports.getAllSocialLinks = async(req, res) =>{
  ...
  const links = await dbConnection.query(
    accountSqlQuery.GET_ALL_SOCIAL_LINKS,
    [req.user._id]
  );
...
module.exports.getAuthorProfileData = async (req, res) => {
  ...
  const authorProfileData = await dbConnection.query(accountSqlQuery.GET_AUTHOR_PROFILE_DATA, [
    req.params.id,
  ]);
  ...
  const authorSocialLinks = await dbConnection.query(accountSqlQuery.GET_AUTHOR_SOCIAL_LINKS, [
    req.params.id,
  ]);
